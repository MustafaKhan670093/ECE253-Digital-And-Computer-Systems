...
SWEEP:		    LDR		R0, =PATTERNS
				LDR		R1, =SEQUENCE_POINTER
				
                LDR		R3, [R1] // load pointer
                LDR     R2, [R0,R3] // load current pattern
                
                ADD R3, R3, #4 // increment pointer
	
                CMP R3, #32 // check that we don't overflow our pointer
                
                BLT DONE_SWEEP // go load next value
                
                MOV R3, #0 // reload pointer if it overflows
                B DONE_SWEEP // go load next value

DONE_SWEEP:
                STR R3, [R1] // put location pointer into memory
                
                CMP R3, #16
				
				PUSH {R2} // push R2 -- current pattern
                
                BLT ADJ_CENTRE // if R3 < 16, then we're moving inward
...
DATA:  
                // fake tasks so it looks like I wrote the code right i.e. memory stuff looks right
				
				POP {R3} // take pattern previously stored in R2 and bring it into R3
				
                LDR		R0, =LEDR_DIRECTION	// put shifting direction
				LDR		R1, =LEDR_PATTERN	// put LEDR pattern into memory
                
                STR		R2, [R0]	// put shifting direction back into memory
                STR		R3, [R1]	// put LEDR pattern into memory           
...
CHK_KEY3:		
				LSR R1, #3 // shift 3 bits right, so that we are inspecting only KEY3.
				AND R1, R1, #1 // mask out only the rightmost bit

				CMP R1, #1 // check to see if key 3 was pressed
                BNE END_KEY_ISR // if not, end the subroutine

START_STOP:		LDR		R0, =MPCORE_PRIV_TIMER		// timer base address
				LDR		R1, [R0, #0x8]		// read timer control register
				EOR     R1, R1, #0b1 // flip enable bit
				STR 	R1, [R0, #0x8] // store timer configuration again
				
				// restart timer at 50 mil
				LDR		R0, =0xFFFEC600 // Timer base address
				LDR R1, =50000000 // 0.25 second = 50 million counts of timer
				STR R1, [R0] // load register value
...
*/
			.global	_start
_start:
			MOV R1, #0b10010 // write CPSR s.t. we enter IRQ mode
            MSR CPSR, R1 // IRQ mode
            LDR SP, =0x20000 // initialize stack pointer in IRQ mode

            MOV R1, #0b10011 // write CPSR s.t. we enter SVC mode
            MSR CPSR, R1 // Supervisor mode
            LDR SP, =0x10000 // initialize stack pointer in SVC mode

			BL			CONFIG_GIC				// configure the ARM generic interrupt controller
			BL			CONFIG_PRIV_TIMER		// configure the MPCore private timer
			BL			CONFIG_KEYS				// configure the pushbutton KEYs
			
			MOV R1, #0b00010011 // write CPSR s.t. we enable interrupts on the ARM processor
            MSR CPSR, R1 // update CPSR with interrupts enabled
...
CONFIG_PRIV_TIMER:
			...
			LDR R1, =50000000 // 0.25 second = 50 million counts of timer
			STR R1, [R0] // load register value
			MOV R1, #0b111 // write configuration register; E = 1, A = 1, I = 1
			STR R1, [R0, #8] // store register
			MOV PC, LR 	// return
...
CONFIG_KEYS:
            ...
            MOV R1, #0b1000 // write interrupt mask
            STR R1, [R0, #8] // store interrupt mask
...
PATTERNS:
	.word 0x201, 0x102, 0x084, 0x048, 0x030, 0x048, 0x084, 0x102
